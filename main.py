...
from tool import rotation
from tool import rotation2
...
# 选择img，annotation的源地与目的地
def imgaug(IMG_DIR, XML_DIR, AUG_IMG_DIR, AUG_XML_DIR):
    boxes_img_aug_list = []
    new_bndbox_list = []

    # 每张影像增强的数量（后面每个变换都设置特定数量？）
    AUGLOOP = 10

    # 翻转控制
    c_contrast_horizon = True
    c_contrast_vertical = True
    # 反色变换
    c_invert = False
    # 像素增减
    c_increase = True
    # 图像压缩
    c_jpegCompression = True
    # 高斯模糊
    c_GaussianNoise = True
    # 丢弃像素
    c_CoarseDropout = True
    # 对比度增强
    c_ContrastNormalization = True
    # 仿射变换
    c_Affine = True
    # X轴平移
    c_translate_x = True
